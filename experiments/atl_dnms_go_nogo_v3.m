...
vr.name = 'test'; % DO NOT FORGET TO CHANGE THE NAME BEFORE EXPERIMENT AND THE DATA FILE NAME AT THE END !
vr.cagenum = 'test'; % Enter cage number
% C249376 M11
% C258735 F20 (death) F21 F22
% C266941 F21(R) F22(R) F25
% C266148 F28 F29(grabs) F30(not licking)

vr.punishment = true; % if this is set to true than some kind of punishment will be present the type of the punishment will be determined later in the decision period code

vr.puff_punishment = true; %deliver air puff for nogo licks
...
vr.correct_right_bias = false; % If this is set to true it will do vice versa compared to if previous is set to true. Setting this to true makes sense only for randomly interleaved trials a
...
if vr.punishment == true
    if vr.puff_punishment == true
        save([vr.finalPathname vr.finalname vr.str_punish vr.str_puff '.mat'],'exper');
    else
        save([vr.finalPathname vr.finalname vr.str_punish '.mat'],'exper');
    end
else
    if vr.puff_punishment == true
        save([vr.finalPathname vr.finalname vr.str_puff '.mat'],'exper');
    else
        save([vr.finalPathname vr.finalname '.mat'],'exper');
    end
end
...
vr.trials = 40; % Enter number of trials 
...
if (round(vr.move,1) == 5.4) && (vr.odor1_end == 1)
...
if ((round(vr.move,1) == 10) && (vr.ITI_happened == 1)) %&& (vr.wrong_picked_left == 0) && (vr.wrong_picked_right == 0)
    disp('ITI ended');
    disp('------------------------------');
    if ((vr.wrong == 1))
...
if vr.punishment == true
    if vr.puff_punishment == true
        save([vr.filename1 '_Data' vr.str_punish vr.str_puff '.mat'],'exper');
    else
        save([vr.filename1 '_Data' vr.str_punish '.mat'],'exper');
    end
else
    if vr.puff_punishment == true
        save([vr.filename1 '_Data' vr.str_puff '.mat'],'exper');
    else
        save([vr.filename1 '_Data' '.mat'],'exper');
    end
end
